...
//Helper Function to revbalidatepaths many more
const revalidatePaths = (paths: string[]) => {
  paths.forEach((path) => revalidatePath(path));
};

//Helper Function with arcjet to limit accsess when user try to upload a video more than two time immeditly
...
//This function requests Bunny.net to create a new video entry and returns the upload URL along with the video GUID and access key for further streaming setup.
...
//Generates a unique timestamp-based thumbnail file path and returns both the upload URL and the CDN-accessible URL, along with the storage access key.
...
//save video detaisl to db and connenct to dbunny and svae infoemation to that specififc video
